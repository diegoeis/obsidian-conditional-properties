...
*Repo note: this repo's frontmatter reads go through `app.metadataCache.getFileCache(file).frontmatter`, writes go through `_writeFrontmatter()` (`processFrontMatter()` / `parseYaml`/`stringifyYaml`), title/path renames go through `fileManager.renameFile()` with path sanitization against vault escape (see commit `cfdd43f`), and deletions — if ever added — should go through `fileManager.trashFile()` as shown below.*
