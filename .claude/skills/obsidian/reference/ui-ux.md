...
*Repo note: this plugin's commands are `conditional-properties-run-now` ("Run conditional rules on vault") and `conditional-properties-run-current-file` ("Run conditional rules on current file") — good examples of sentence-case naming, though note Obsidian already auto-namespaces with the plugin ID, so a fresh command added here shouldn't repeat `conditional-properties-` in its own `id`.*
