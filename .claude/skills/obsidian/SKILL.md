...
name: obsidian
version: 1.1.0
description: Comprehensive guidelines for Obsidian.md plugin development including the official Obsidian ESLint rules, memory management, API usage (requestUrl vs fetch, processFrontMatter, renameFile/trashFile), UI/UX standards, and submission requirements. Use when working with Obsidian plugins, main.js/main.ts files, manifest.json, Plugin class, MarkdownView, TFile, vault operations, or any Obsidian API development.
---
...
**Note for this repo (Conditional Properties):** this plugin ships compiled JavaScript directly — `main.js` is hand-edited, there is no `main.ts`, no build step, and no TypeScript compiler. Code examples in this skill and its reference files are written in TypeScript because that's the idiom the official Obsidian ESLint rules are documented in, but every API mentioned (`Plugin`, `TFile`, `Setting`, `processFrontMatter`, `registerInterval`, etc.) is the same runtime API this repo calls from plain JS — drop the type annotations and the guidance still applies. Where a rule is genuinely TypeScript-only (e.g. avoiding `any`), see `reference/type-safety.md` for the JS-equivalent principle.

## Getting Started

This section (boilerplate generator, `/create-plugin`) describes tooling from a different, TypeScript-based Obsidian plugin template and does not apply to this repo. Conditional Properties has no scaffolding step — to make a change, edit `main.js` directly and run `./scripts/sync.sh` to reload it in the test vault (see the root `CLAUDE.md`).
