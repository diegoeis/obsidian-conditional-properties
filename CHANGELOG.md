...
## 0.26.0 - 2026-09-04
### Added
- **IF condition values now accept placeholders.** Every placeholder that already worked in a THEN action's value (`{{today}}`, `{{filename}}`, `{{title}}`, `{{created_date}}`, `{{updated_date}}`, `{{time}}`, and `{{propertyName}}` for any other frontmatter property) now also resolves inside a Property, First level title, or Note file condition's value, before the comparison runs — e.g. `Property: dateDue → exactly match → "{{today}}"`, or comparing one property against another with `Property: type → exactly match → "{{company}}"`. Resolution is skipped for a `/regex/`-mode value (expanding a placeholder inside a regex pattern is out of scope for now) and for a non-string value. Note file conditions get the same date-only forcing (`YYYY-MM-DD`, never a time component) that Note file THEN actions already had, since filenames can't safely hold a `:`.
- **`{{yesterday}}` and `{{tomorrow}}` placeholders**, alongside the existing `{{date}}`/`{{today}}`. Work in both THEN actions and IF conditions, accept the same `:FORMAT` suffix (`{{yesterday:DD-MM-YYYY}}`), and support both the double-brace and legacy single-brace syntax.
- **Rule search now also searches THEN action fields, not just the IF condition.** Searching "Property" used to match only the IF condition's property name (`ifProp`); it now also matches the IF condition's value, and the matching THEN property action's property name, value, and rename target. "First level title" and "Note file" searches likewise now also match the corresponding THEN action's text (and bookmark group, for Note file).
- **Onboarding empty state.** With zero rules — a fresh install, or every rule deleted — the search bar and rule list are replaced by a short welcome message ("Welcome! Lets automate your notes!") and its own "Add rule" button, centered and capped at 500px wide. Disappears the moment a rule exists (from either this button or the heading's own "Add rule"), and reappears if you delete your way back down to zero.
- The remove ("×") button on a THEN action row now only shows once there's more than one action in the rule — mirrors the same convention IF condition rows already followed, since a rule always needs at least one action.

### Fixed
- **Docs site: `conditions/note-file.md` was missing the `{% raw %}...{% endraw %}` wrap** that every other page referencing `{{...}}` placeholders already had, so Jekyll's Liquid engine was silently eating every placeholder example on that page in production (rendering as empty text). Also fixed several broken internal links on the docs site home page (missing the site's `/obsidian-conditional-properties` baseurl prefix) and the "This site uses Just the Docs" theme attribution still showing after an empty override (the theme treats a fully-empty override as "none provided" — hidden via CSS instead).

## 0.25.4 - 2026-08-29
### Changed
- **Condition/action rows restyled.** Each rule's card lost its border, keeping only a background fill (`var(--setting-items-background)`) and padding; every condition/action row now reads as one line — label, fields, and remove button inline instead of stacked. Row labels changed from "Condition 1" / "Condition 2" / "Action 1" to a sentence-like "Where" (first condition), "Or" / "And" (every following condition, matching the rule's Any/All setting), and "Do this" (every action) — a fixed-width, right-aligned column to the left of each row's fields. Every field inside a row (text inputs, selects, dropdowns) gets a contrast fill via `var(--background-modifier-form-field)` — Obsidian's own input/select background token — since the row itself no longer carries one.
