...
*Repo note: examples below use TypeScript syntax; this repo's `main.js` is plain JS with no build step, so apply the same API guidance without the type annotations. The "organize into folders" pattern under Miscellaneous Rules does not apply here — this repo intentionally ships a single-file `main.js` (see root `CLAUDE.md`).*
