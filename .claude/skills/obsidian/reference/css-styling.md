...
*Repo note: this repo's styles live in the single `styles.css` at the project root, scoped with `conditional-properties-*` class prefixes rather than the `my-plugin-*`/`my-todo-plugin-*` placeholders used in the examples below.*
