...
*Repo note: examples below use TypeScript syntax; in this repo's plain-JS `main.js` the same APIs apply without type annotations, e.g. the rule-editor rows and buttons built in `ConditionalPropertiesSettingTab` with `createEl`/`Setting`/`ButtonComponent`.*
